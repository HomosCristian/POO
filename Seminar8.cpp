...
// Constructori
	...
	// Aplicatie: are note -> fiecare nota se modifica prin: nota= 10 - nota, retirn true
	// nu are note -. return false
...
public:
	...
	char* getStr()
	{
		...
		strcpy_s(tmp, strlen(this->str), this->str);
		return this->str;
	}
}; */
int main()
{
	/* MyString s1("Testare pointer char");
	char* x = s1.getStr();
	...
	Student s1 = s; //se apeleaza constructorul de copiere pt ca s1 nu exista se creeaza acum
	Student s2;
	s2 = s1; */
	...
	Student* ps;  //adresa obiectului s
	ps = &s;  // ps retine adresa lui s
	cout << (*ps).nume << endl;
	cout << ps->nume << endl;
	cout << sizeof(s) << endl;
	...
	Student s6(s5);   // se apeleaza operatorul de copiere 
	...
	s = s5 = s6;  // mai intai se copiaza s6 in s5
