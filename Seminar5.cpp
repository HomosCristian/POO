...
class Student
{
	...
	int varsta;
	int an_studii;
public:   //deasupra e private ca nu e specificat nimic
	...
	Student(string _nume, char* _adresa, int _varsta, int _an_studii, int* _note, int _numar_materii, string _numar_matricol, finantare _finantare) :
		nume(_nume), varsta(_varsta), an_studii(_an_studii), numar_matricol(_numar_matricol), finantare(_finantare), numar_materii(_numar_materii)  //atribuire de valori dar e acelasi lucru ca si this->nume=_nume etc
	{
		//care este diferenta?
		// 
		//this->adresa = _adresa;
		...
		//this->set_note(_note, _numar materii);  //e acelasi lucru ca urm 3 linii de cod
   ...
   //setters
	void set_note(int* _note, int _numar_materii)
	{
		...
		if (this->note != nullptr)
			delete[] this->note;   // eliberam memoria pt alte valori
		this->note = new int[_numar_materii];
		for (int index = 0; index < _numar_materii; index++)
		{
			this->note[index] = _note[index];
	...
	void set_adresa(char* _adresa)
	{
		...
		strcpy_s(this->adresa, 20, "Bucuresti, Sector 2");
	}
	...
	//getters - functie publica pt accesarea chestilor private
...
int main()
{
	Student s0;   //se apeleaza primul contructor

	//s0.nume = "Anghel Mihai";

	//s0.adresa = new char[20];   // e de tip char* si trrebuie sa alocam memorie ca sa mearga
	...
	//s0.note = new int[20];   pt ca membrul note este privat

	s0.set_nume("Cristi");
	s0.set_adresa((char*)"1234");
	s0.set_varsta(21);
	s0.set_an_studii(2023);
	...
	Student s1 = Student("Popescu I", (char*)"Bucuresti, Sector 1", 21, 2, note, 10, "A689867", finantare::BUGET_BURSA);  //se apeleaza al doilea constructor
